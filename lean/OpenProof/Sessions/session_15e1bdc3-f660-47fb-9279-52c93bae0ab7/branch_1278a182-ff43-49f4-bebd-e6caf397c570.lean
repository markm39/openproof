...
Updated: 
-/
-- branch: Fibonacci Gcd Identity Over `Nat` repair [proving]
-- branch id: branch_1278a182-ff43-49f4-bebd-e6caf397c570
-- node: Fibonacci Gcd Identity Over `Nat` [verifying]
-- kind: theorem
-- statement: `theorem fib_gcd (m n : ℕ) : Nat.gcd (Nat.fib m) (Nat.fib n) = Nat.fib (Nat.gcd m n)`

-- artifact: Fibonacci Gcd Identity Over `Nat` repair [pending]
-- artifact id: artifact_branch_1278a182-ff43-49f4-bebd-e6caf397c570
-- artifact kind: snippet

-- target kind: artifact
-- target id: artifact_branch_1278a182-ff43-49f4-bebd-e6caf397c570
-- label: Fibonacci Gcd Identity Over `Nat` repair
...
example
  ...
  rw [ih (m % n) (Nat.mod_lt m hn) n]
  rw [Nat.gcd_comm n (m % n)]
  rw [← Nat.gcd_rec n m]
  rw [Nat.gcd_comm n m]